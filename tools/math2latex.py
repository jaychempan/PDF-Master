...
def create_latex_documents_from_json(json_path, result_pdf_path):
    ...
    result_doc.packages.append(Package('amsmath'))
    result_doc.packages.append(Package('amssymb'))
    result_doc.packages.append(Package('amsfonts'))
    ...
    for idx, item in enumerate(data):
        ...
        output_path = os.path.join('maths', f'{image_name}')
...
if __name__ == '__main__':
    json_path = 'sample_equation.json'  # 替换为你的JSON文件路径
    result_pdf_path = os.path.join('maths', 'math_compilation_results')  # 结果PDF输出路径
